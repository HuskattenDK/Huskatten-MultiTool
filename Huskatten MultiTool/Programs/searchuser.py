...
def Continue():
    input(f"\n{Colors.red}Tryk Enter for at fortsætte...")

def clear():
    print("\033c", end="")

def OnlyWindows():
    print(f"{Colors.red}Denne mulighed er kun tilgængelig på Windows.")
    Continue()
    clear()

def OnlyLinux():
    print(f"{Colors.red}Denne mulighed er kun tilgængelig på Linux.")
    Continue()
    clear()

def ErrorChoice():
    print(f"{Colors.red}Ugyldigt valg. Vælg venligst en gyldig mulighed.")
    Continue()
    clear()

def ErrorModule(e):
    print(f"{Colors.red}Fejl ved import af modul: {e}")
    Continue()
    clear()
...
def tiktok_search(driver, username):
    try:
        ...
        time.sleep(2)
        if "Denne konto kan ikke findes" in text_translated(driver.execute_script("return document.documentElement.innerText")):
            return False
        else:
            return link
